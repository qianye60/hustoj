			...
			success: function(data) {
				...
				if(title==""){
					...
					try {
						...
						// 使用KindEditor API设置富文本编辑器的值
						let editors = KindEditor.instances;
						if(editors.length >= 4) {
							editors[0].html("<span class='md'>"+json.description+"</span>");
							editors[1].html("<span class='md'>"+json.input+"</span>");
							editors[2].html("<span class='md'>"+json.output+"</span>");
							editors[3].html("<span class='md'>"+json.hint+"</span>");
						}
						// 普通textarea直接设置
						$("textarea[name='sample_input']").val(json.sample_input);
						$("textarea[name='sample_output']").val(json.sample_output);
					} catch(e) {
						console.log("JSON解析失败:", e);
						// 如果不是JSON，按原来方式处理
						if(KindEditor.instances.length > 0) {
							KindEditor.instances[0].html("<span class='md'>"+data+"</span>");
						}
					}
				}
				window.setTimeout(function(){
					// 同步KindEditor内容到textarea
					KindEditor.each(KindEditor.instances, function(i, editor) {
						editor.sync();
					});
					sync();
				}, 500);
